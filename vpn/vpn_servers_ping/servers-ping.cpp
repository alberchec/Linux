...
int main(){
	ifstream input("home/alberto/files/linux/vpn_servers_ping/servers-names");
	string server;
	...
	for(int i=0;i<servers.size();i++){
		...
		string cmd = "ping -c 1 " + servers[i].name + ".privacy.network | grep \"64 bytes\" > /home/alberto/files/linux/vpn_servers_ping/" + servers[i].location + ".txt";
		system(cmd.c_str() );
		cout << "\b\b\b" << " OK\n";
	}

	for(int i=0;i<servers.size();i++){
		input.open("/home/alberto/files/linux/vpn_servers_ping/" + servers[i].location + ".txt");
	...
	system("rm /home/alberto/files/linux/vpn_servers_ping/*.txt");
}
